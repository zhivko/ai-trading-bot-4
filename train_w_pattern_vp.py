...
def prepare_data(path_1h, window_size=100, lookforward=24):
    ...
    print("Generating Features (TS + Volume Profile)... This may take 2-3 mins.")
    
    for i in tqdm(range(200, len(df) - lookforward)): # Start at 200 for VP lookback
        # 1. Extract Window for CNN (Stochastic)
        win_long = df['stoch_long'].iloc[i-window_size:i].values
        
        if np.isnan(win_long).any(): continue

        # 2. Extract Volume Profile (Context)
        # Lookback 200 bars for market structure
        ...
        # 3. Downtrend Channel Logic (Labeling)
...
def train_trading_bot(X_ts, X_vp, y):
    ...
    torch.save(model.state_dict(), "stoch_vp_detector_5090.pth")
    print("Model saved to stoch_vp_detector_5090.pth.")
