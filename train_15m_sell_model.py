...
def get_channel_metrics(prices):
    """Returns slope and R-squared to define the 'Quality' of a trend."""
...
def prepare_data(path, window_size=100, lookforward=32):
    ...
    # Volatility Adjustment (Inverse unused but keeping structure consistent)
    df['atr_ratio'] = df['atr'] / df['atr'].rolling(200).mean()
    
    data_x_ts, data_x_vp, data_y = [], [], []
    
    print("Generating Features (TS + Volume Profile) for SELL signals... This may take a few minutes.")
    
    # Validation counters
    count_skipped_vp = 0
    count_skipped_nan = 0
    
    # Step size: 2 to reduce redundancy if dataset is huge
    step = 2 
    ...
    for i in tqdm(range(200, len(df) - lookforward, step)): 
        ...
        # 3. Uptrend Channel Logic (Labeling for SELL)
        price_window = df['close'].iloc[i-window_size:i]
        slope, r_sq = get_channel_metrics(price_window)
        
        # SELL CRITERIA:
        # - Uptrend (slope > 0)
        # - Stochastic high (overbought)
        # - Future return < -1.5% (dump)
        
        avg_stoch_recent = np.mean(win_long[-20:])
        is_high_stoch = avg_stoch_recent > 0.70  # Average above 70% is "high enough"
        
        max_stoch_recent = np.max(win_long[-20:])
        has_peak = max_stoch_recent > 0.80  # At least one touch above 80%
        ...
        # - Slope > 0 (Uptrend)
        # - R² > 0.35 (Clear trend)
        # - Either high average OR clear peak
        # - Future return < -1.5%
        if slope > 0 and r_sq > 0.35:
            if (is_high_stoch or has_peak) and future_return < -0.015:
                label = 1
...
def train_trading_bot(X_ts, X_vp, y):
    ...
    print(f"Training SELL Detector on {device}")
    ...
    if pos_count == 0:
        print("CRITICAL ERROR: No positive examples found for SELL signals. Relax labeling criteria.")
        return

    pos_weight_value = (neg_count / pos_count) * 2.0
    pos_weight = torch.tensor([pos_weight_value]).to(device)
    
    print(f"\nClass Distribution (SELL Signals):")
    ...
    # SAVE WITH SELL NAME
    torch.save(model.state_dict(), "stoch_vp_sell_detector_15m.pth")
    print("Model saved to stoch_vp_sell_detector_15m.pth.")
