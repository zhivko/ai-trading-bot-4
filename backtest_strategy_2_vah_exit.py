...
def execute_trade(df, entry_idx, current_balance, position_size_pct=0.5, precise: bool = False, is_sell: bool = False):
    """
    Execute a single trade using Strategy 2: Hybrid VAH/VAL PPT + Trail
    """
    ...
    # Determine PPT Level (VAH for Longs, VAL for Shorts)
    ppt_price = None
    if is_sell:
        if vp['val'] < entry_price * 0.99: ppt_price = vp['val']
        elif vp['poc_price'] < entry_price * 0.99: ppt_price = vp['poc_price']
        else: ppt_price = entry_price * 0.96
    else:
        if vp['vah'] > entry_price * 1.01: ppt_price = vp['vah']
        elif vp['poc_price'] > entry_price * 1.01: ppt_price = vp['poc_price']
